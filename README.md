...
5.  **Migrações (obrigatório para emissão):**
    O próximo número da DPS e o histórico de notas são armazenados no banco. Rode as migrações antes de emitir:
    ```bash
...
* `app/Models`: `NfseSequencia` (próximo número DPS por série), `NfseEmitida` (histórico das notas emitidas).
...
O próximo número da DPS é obtido automaticamente pela tabela `nfse_sequencia` (por série); o histórico de emissões fica em `nfse_emitidas`. Não é mais necessário configurar `NFSE_NUMERO_DPS` no `.env`.
